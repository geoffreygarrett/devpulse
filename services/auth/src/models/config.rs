...
fn get_env_var_with_default<T: std::str::FromStr + std::fmt::Display>(key: &str, default: T) -> T {
    match std::env::var(key) {
        ...
        Err(_) => {
            warn!("{} not provided, using default value: {}", key, default);
            default
        }
    }
}
...
fn get_env_var_optional(key: &str) -> Option<String> {
    std::env::var(key)
        ...
        .or_else(|| {
            warn!("{} is not set, using default value: None", key);
            None
        })
}
