...
use devpulse_db::services::v1::account::PostgresAccountService;
use devpulse_db::services::v1::refresh_token::PostgresRefreshTokenService;
...
mod v1 {
    ...
    pub use devpulse_db::account::v1::account_service_server::AccountServiceServer;
    pub use devpulse_db::refresh_token::v1::refresh_token_service_server::RefreshTokenServiceServer;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let account_service = PostgresAccountService::new(&pg_pool);
    let refresh_token_service = PostgresRefreshTokenService::new(&pg_pool);
    Server::builder()
        .add_service(
        ...
        .add_service(v1::AccountServiceServer::new(account_service))
        .add_service(v1::RefreshTokenServiceServer::new(refresh_token_service))
            // .add_service(v1::IdentityServiceServer::new(services.identity_service))
            // .add_service(v1::RefreshTokenServiceServer::new(services.refresh_token_service))
            .serve(addr)
            .await?;
