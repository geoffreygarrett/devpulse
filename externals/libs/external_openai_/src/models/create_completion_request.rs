...
pub struct CreateCompletionRequest {
    ...
    /// Modify the likelihood of specified tokens appearing in the completion.  Accepts a JSON object that maps tokens (specified by their token ID in the GPT tokenizer) to an associated bias value from -100 to 100. You can use this [tokenizer tool](/tokenizer?view=bpe) to convert text to token IDs. Mathema
    #[serde(
        rename = "logit_bias",
        default,
        with = "::serde_with::rust::double_option",
    ...
    /// Include the log probabilities on the `logprobs` most likely output tokens, as well the chosen tokens. For example, if `logprobs` is 5, the API will return a list of the 5 most likely tokens. The API will always return the `logprob` of the sampled token, so there may be up to `logprobs+1` elements in
    #[serde(
        rename = "logprobs",
        default,
        with = "::serde_with::rust::double_option",
    ...
    /// If specified, our system will make a best effort to sample deterministically, such that repeated requests with the same `seed` and parameters should return the same result.  Determinism is not guaranteed, and you should refer to the `system_fingerprint` response parameter to monitor changes in the b
    #[serde(
        rename = "seed",
        default,
        with = "::serde_with::rust::double_option",
...
impl CreateCompletionRequest {
    pub fn new(
        ...
        prompt: Option<models::CreateCompletionRequestPrompt>,
    ) -> CreateCompletionRequest {
        CreateCompletionRequest {
            model: Box::new(model),
            prompt: if let Some(x) = prompt {
                Some(Box::new(x))
            } else {
                None
            },
    ...
    pub fn builder() -> CreateCompletionRequestBuilder {
        CreateCompletionRequestBuilder::create_empty()
    }
}
