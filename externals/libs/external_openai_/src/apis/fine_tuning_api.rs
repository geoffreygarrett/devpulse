...
/// struct for passing parameters to the method [`create_fine_tuning_job`]
#[derive(Clone, Debug, Builder, Default, Serialize, PartialEq, Eq)]
#[builder(setter(strip_option, into), default)]
pub struct CreateFineTuningJobParams {
    pub create_fine_tuning_job_request: models::CreateFineTuningJobRequest,
}

impl CreateFineTuningJobParams {
    pub fn builder() -> CreateFineTuningJobParamsBuilder {
        CreateFineTuningJobParamsBuilder::default()
    }
}
...
pub async fn create_fine_tuning_job(
    configuration: &configuration::Configuration, params: CreateFineTuningJobParams,
) -> Result<models::FineTuningJob, Error<CreateFineTuningJobError>> {
    let local_var_configuration = configuration;

    // unbox the parameters
    let create_fine_tuning_job_request = params.create_fine_tuning_job_request;

    debug!("Calling create_fine_tuning_job...");
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateFineTuningJobError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
