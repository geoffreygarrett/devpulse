...
#[macro_export]
macro_rules! generate_moderations_api_client_methods {
    () => {
        /// create_moderation method
        pub async fn create_moderation(
            &self, params: CreateModerationParams,
        ) -> Result<models::CreateModerationResponse, Error<CreateModerationError>> {
            let result =
                crate::apis::moderations_api::create_moderation(&self.config, params).await?;
            Ok(result)
        }
    };
}
...
#[macro_export]
macro_rules! generate_moderations_api_client {
    () => {
        use cached::SizedCache;
        use reqwest::Client;
        use std::sync::Arc;
