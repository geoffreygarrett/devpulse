...
pub struct CreateBatchRequest {
    /// The ID of an uploaded file that contains requests for the new batch.  See [upload file](/docs/api-reference/files/create) for how to upload a file.  Your input file must be formatted as a [JSONL file](/docs/api-reference/batch/request-input), and must be uploaded with the purpose `batch`. The file c
...
impl CreateBatchRequest {
    pub fn new(
        input_file_id: String, endpoint: Endpoint, completion_window: CompletionWindow,
    ) -> CreateBatchRequest {
        CreateBatchRequest {
            input_file_id,
            endpoint,
    ...
    pub fn builder() -> CreateBatchRequestBuilder {
        CreateBatchRequestBuilder::create_empty()
    }
}
/// The endpoint to be used for all requests in the batch. Currently `/v1/chat/completions`, `/v1/embeddings`, and `/v1/completions` are supported. Note that `/v1/embeddings` batches are also restricted to a maximum of 50,000 embedding inputs across all requests in the batch.
